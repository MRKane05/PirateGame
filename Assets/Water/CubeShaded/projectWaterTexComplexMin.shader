...
Shader "Water/projectWaterTexComplexMin" {
	Properties {	
	...
	SubShader {
		...
		Pass {
			...
			float3 bilinearDirectionPosition(fixed2 position, out fixed dist, out fixed3 viewDir) {
				...
				viewDir = fixed3(lerpC);	//we get this for "free"
			...
			fragmentInput vert(vertexInput v) {
				...
				height = (height + tex2Dlod(_Offsetmap, float4(o.rippleCoords.zw, 0, lod)).a);
				...
				*/
				...
				//peakUVo.peakUV = o.floorUV.xzxz * _FoamTiling.xyzw;// + _foamTransOffset.xyzw;	//Used for our foam wave peaks
			...
			inline fixed3 combineNormalMaps(fixed4 base, fixed4 detail) {
				base += fixed4(0,0,1,0);
				detail *= fixed4(1, 1, -1,1);
			...
			float4 frag(fragmentInput i) : COLOR {
				...
				float3 worldNormal = (combineNormalMaps(bumpFar, bump)); //(bump+bumpFar)*0.5; //
          		//float3 reflectVector = reflect(-i.viewDir), worldNormal.xzy); //worldNormal));
				float4 waterCube = texCUBE(_WaterCube, -worldNormal.xzy); // reflectVector.xyz); //create and shade our surface
				float fresnel = saturate(tex2D(_fresnelTex, float2(dot(-i.viewDir, worldNormal.xzy), 1)).a);	//Different due to the normal combine being abbreviated on different axes
				
				waterCube = lerp(_Color, waterCube, fresnel);	//Include a brutally simple non-paralax approach to the distortion
				//waterCube += (tex2D(_FoamTex, i.peakUV.xy)+tex2D(_FoamTex, i.peakUV.zw))*i.peakColor;	//Wave peaks. not strictly necessary
				return waterCube*tex2D(_LightingTex, worldNormal.xy); //Shading Pass
				//return color;
			}
	...
	Fallback "Water/projectWaterTex"
}