...
Shader "Water/projectWaterTexComplexTransluscent" {
	Properties {	
		_Color("Water Base Color (base additive)", Color) = (0,0,0,1)
		_transColor("Translucent Colour", Color) = (0.80784,.968627,1,1)
	...
	SubShader {
		...
		Pass {
			...
			fixed4 _Color, _transColor;
			samplerCUBE _WaterCube, _SunCube; //, _LightCube;
			...
			fragmentInput vert(vertexInput v) {
				...
				//This should now have a maximum of 2...
				//height = height-1.0;
				...
				//lets see about our smaller wavelets
				o.bumpcoord = o.floorUV.xzxz * _BumpTiling.xyzw + _bumpTransOffset.xyzw; //used for the bumpmap	
				//height = height+(tex2Dlod(_Offsetmap, float4(o.bumpcoord.xy, 0, lod)).a-0.5);
				//height = height + (tex2Dlod(_Offsetmap, float4(o.bumpcoord.zw, 0, lod)).a-0.5);	//This isn't operating correctly on android...


				//height = (height/2.0);
				//height = height/2.1;
				v.vertex += _PeakDirection*height*_WaveHeight;
				//v.vertex += float4(0,-1,0, 0)*(_WaveHeight*0.5);
				...
				o.heightColor = saturate(height-0.5)*(1+o.viewDir.y)*_transColor/(dist/30);
			...
			*/
			half3 UnpackGetBump(sampler2D bumpTexture, half4 uv) {
				half3 bump = tex2D(bumpTexture, uv.xy) + tex2D(bumpTexture, uv.zw);	//Probably not necessary, but I like the small change that it creates
				return (bump*2-2)*0.5;
			}

			half3 UnpackGetBumpRipped(sampler2D bumpTexture, half4 uv) {
				half3 bump = tex2D(bumpTexture, uv.xy);
				bump = bump + tex2D(bumpTexture, uv.zw-bump.xz*0.05);	//Probably not necessary, but I like the small change that it creates
				return (bump*2-2)*0.5;
			}
			...
			float4 frag(fragmentInput i) : COLOR {
				//float4 color=_Color;							
				fixed3 bump = UnpackGetBumpRipped(_BumpMap, i.bumpcoord.xyzw);
		        ...
		        fixed4 foamCol = (tex2D(_FoamTex, i.peakUV.xy)*tex2D(_FoamTex, i.peakUV.zw))*i.peakColor;	//Wave peaks. not strictly necessary
				//Complex fresnel that takes into account the wave foam distortion
				fixed fresnel = saturate(tex2D(_fresnelTex, float2(dot(-i.viewDir, worldNormal.xzy), 0.5)).a)*(1.0-foamCol.a);	//Different due to the normal combine being abbreviated on different axes

				fixed4 color = tex2D(_MainTex, i.floorUV.xz+worldNormal.xzy*_floorDistort)+i.heightColor;
				color = lerp(color, texCUBE(_WaterCube, reflectVector.xyz), fresnel);	//Add in our reflection for this
				color += texCUBE(_SunCube, reflectVector.xyz)*fresnel*2.0; //add our sun specular pickup
				color += foamCol;
