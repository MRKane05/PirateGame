...
Shader "Water/projectWaterVita" {
	Properties {	
		...
		_Offsetmap ("WaveOffsetAlphas", 2D) = "white" {}	//This could be collapsed down into the bump map
		_WaveHeight ("WaveHeight", Range(0.0, 20.0)) = 2.0
	...
	SubShader {
		...
		Pass {
			...
			sampler2D _MainTex, _BumpMap, _Offsetmap, _FoamTex, _LightingTex, _WaterTexture;
			...
			inline half3 combineNormalMaps(fixed3 base, fixed3 detail) {
				base += fixed3(0,0,1);
				detail *= fixed3(-1, -1, -1);
				return (base * dot(base, detail)/base.z - detail);			
			}

			//Because the Vita seems to have issues with the reflection standard code?
			half3 vecReflect(half3 incidentVec, half3 normal)
			{
			...
			fragmentInput vert(vertexInput v) {
				...
				v.vertex.y += _WaveHeight * height * (1.0f / clamp(dist*dist, 1.0, 100.0));
			...
			float4 frag(fragmentInput i) : COLOR{
				...
				//half3 bump = UnpackGetBump(_BumpMap, i.rippleCoords.xyzw);
				half3 bump = UnpackNormal(tex2D(_BumpMap, i.rippleCoords.xy));
				bump = (bump + UnpackNormal(tex2D(_BumpMap, i.rippleCoords.zw + bump.xy*0.05f))) * 0.25;

				//half3 worldNormal = combineNormalMaps(i.vtxNormalWorld.xyz, bump);
				half3 worldNormal = bump; //(combineNormalMaps(bumpFar, bump));
				float4 color = tex2D(_WaterTexture, i.bumpcoord.xy + bump.xy*0.25f);
				/*
				bump = UnpackNormal(tex2D(_BumpMap, i.bumpcoord.xy));
				bump = (bump + UnpackNormal(tex2D(_BumpMap, i.bumpcoord.zw))) * 0.5;

				worldNormal = combineNormalMaps(worldNormal, bump);
				*/

          		//worldNormal.y = 0;
          		half3 reflectVector = vecReflect(-i.viewDir, worldNormal.xzy); //worldNormal));
		        fixed4 waterCube = texCUBE(_WaterCube, reflectVector); //create and shade our surface
				...
				//color = tex2D(_LightingTex, normalize(bump).xz)*waterCube;
				//color = texCUBE(_WaterCube, -worldNormal);
				
				color += waterCube; //Shading pass
