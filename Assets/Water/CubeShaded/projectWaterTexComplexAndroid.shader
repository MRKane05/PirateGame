...
Shader "Water/projectWaterTexComplexAndroid" {
	Properties {	
	...
	SubShader {
		...
		Pass {
			...
			fragmentInput vert(vertexInput v) {
				...
				//This is the only shader where Androids shortcomings are seriously visible
			...
			float4 frag(fragmentInput i) : COLOR {
				...
				fixed3 worldNormal = (bump+bumpFar)*0.5; //(combineNormalMaps(bumpFar, bump));
          		fixed3 reflectVector = reflect(fixed3(-i.viewDir), fixed3(worldNormal.xzy)); //worldNormal));
		        fixed4 waterCube = texCUBE(_WaterCube, reflectVector.xyz); //create and shade our surface
				fixed fresnel = saturate(tex2D(_fresnelTex, float2(dot(-i.viewDir, worldNormal.xzy), 1)).a);	//Different due to the normal combine being abbreviated on different axes
